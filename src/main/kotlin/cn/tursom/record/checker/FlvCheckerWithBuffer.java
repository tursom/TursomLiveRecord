 ...
 */
@SuppressWarnings({"unused", "ResultOfMethodCallIgnored", "JavaDoc", "AccessStaticViaInstance", "DuplicatedCode", "CommentedOutCode", "UnusedReturnValue"})
public class FlvCheckerWithBuffer {
    // 用于统计时间戳
    protected int[] lastTimestampRead = {-1, -1}, lastTimestampWrite = {-1, -1};
    // 用于缓冲
    private final byte[] buffer = new byte[1024 * 1024 * 4];
    ...
    private void checkTag(RafRBuffered raf, RafWBuffered rafNew, File fileNew, boolean splitScripts,
        ...
        try {
            int remain = 40;
            boolean isFirstScriptTag = !invokedSplitByHeader;
            boolean isFirstAudioHeader = !invokedSplitByHeader;
            boolean isFirstVideoHeader = !invokedSplitByHeader;
